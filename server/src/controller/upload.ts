...
import { deleteFileAsync, createDirectoryAsync, copyFileAsync } from "../lib/file";
...
export interface UPLOAD_INTERFACE { nim: string, name: string, file_name: string, kode_soal?: string | undefined, ip_pc?: number, file_data: FileList }
...
export const uploadFileController = async (req: Request, res: Response) => {
    ...
    /* A timer that will delete the file if the response is not sent within 5 seconds. */
    const interval = setInterval(() => {
        if (!res.headersSent) return
        if (!req.file) return
        deleteFileAsync("./uploads/" + req.file!.filename)
            .catch(err => {
    ...
    if (!useConfig()) return res.status(501).json({ message: "configuration does not exist" })
    ...
    /* A type declaration. */
    const config = useConfig();
    const body: UPLOAD_INTERFACE = req.body;

    /* Checking if the user has already uploaded a file. */
    if (listUpload.get(body.nim)) return res.status(403).json({ message: "user NIM already exists" })
...
export const deleteFileController = async (req: Request, res: Response) => {
    if (listUpload.getAll().length === 0 || !listUpload.get(req.body.nim)) return res.status(404).json({message : "file not exists"})
    /* Deleting a file. */
    deleteFileAsync(useConfig()!.dir + "\\" + listUpload.get(req.body.nim)?.file_name)
        .catch(err => {
            printLog(err, LOGTYPE.failed)
