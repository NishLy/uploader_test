...
export const copyFileAsyncBuffer = (
  src: string,
  dest: string,
  onProgress?: (param: number) => void
) => {
  return new Promise((resolve, rejects) => {
    console.time("waktu copy");
    fs.stat(src, function (err, stat) {
      ...
      readStream.on("data", function (buffer) {
        ...
        let percentage = (bytesCopied / filesize) * 100;
        if (onProgress) onProgress(percentage);
      });

      readStream.on("end", function () {
        resolve(console.log(console.timeEnd("waktu copy")+ "ukuran :" + chalk.red(filesize) + " bytes"));
      });
